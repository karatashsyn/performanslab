...
export default function Navbar({ transparent }) {
  ...
  const navLink = ({ href, label, key }) => (
    <li
      ...
      style={{
        fontWeight: router.asPath === href ? 500 : "",
      }}
      className={`${
        router.asPath === href ? "text-nav-red" : ""
  ...
  const BlogNavbarDecoration = () => (
    <div
      className={`${
        (router.asPath.includes("arsiv") ||
          router.pathname === "/[slug]" ||
          router.pathname === "/category/perftest") &&
        !menuOpen
          ? "flex"
          : "hidden"
  ...
  return (
    <PaddedContainer className={`${transparent ? " " : " !bg-black"} `}>
